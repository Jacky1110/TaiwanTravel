package com.jotangi.nickyen.utils.internal;
...
import com.jotangi.nickyen.utils.ItemController;
import com.jotangi.nickyen.utils.item.BaseTabItem;
import com.jotangi.nickyen.utils.listener.OnTabItemSelectedListener;
import com.jotangi.nickyen.utils.listener.SimpleTabItemSelectedListener;
...
public class CustomItemLayout extends ViewGroup implements ItemController {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            child.measure(childWidthMeasureSpec, childHeightMeasureSpec + 120);
        }
    ...
    //紀錄前第二個選中項
    int pre2oldSelected = -1; // 要改第三個fragment點選不切換導航欄icon顏色新增的
...
//        mItems.get(mSelected).setChecked(true);
        ...
        // 要改第三個fragment點選不切換導航欄icon顏色
        if (oldSelected >= 0) {
            if (mSelected == 2){
                mItems.get(oldSelected).setChecked(true);
                pre2oldSelected = oldSelected;
            }else
            {
                mItems.get(oldSelected).setChecked(false);
            }
        }
        Log.d("快1", "setSelect: "+pre2oldSelected+"/"+oldSelected+"/"+mSelected);
        if (oldSelected == 2 && pre2oldSelected != -1) {
            mItems.get(oldSelected).setChecked(false);
            mItems.get(pre2oldSelected).setChecked(false);
        }
        Log.d("快1", "setSelect2: "+pre2oldSelected+"/"+oldSelected+"/"+mSelected);
        if (mSelected != 2){
            mItems.get(mSelected).setChecked(true);
        }
        // 要改第三個fragment點選不切換導航欄icon顏色
